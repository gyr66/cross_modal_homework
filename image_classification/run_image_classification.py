...
def main():
    ...
    def compute_metrics(p):
        ...
        report = classification_report(p.label_ids, predictions, output_dict=True)
        metrics = {
            f"precision_{id2label[str(label)]}": report[label]['precision'] for label in report if label.isdigit()
        }
        metrics.update({
            f"recall_{id2label[str(label)]}": report[label]['recall'] for label in report if label.isdigit()
        })
        metrics.update({
            f"f1_{id2label[str(label)]}": report[label]['f1-score'] for label in report if label.isdigit()
        })
